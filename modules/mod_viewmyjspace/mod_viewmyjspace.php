...
if (@file_exists(JPATH_ROOT.'/components/com_myjspace/helpers/util.php')) {
	...
	$affmax = intval($params->get('affmax', 20));
	...
	// ACL
	$search_restrict_acl = $params->get('search_restrict_acl', 0); // Restrict list with ACL
	...
	if ($showmode == 1 || $showmode == 2)
		$names = modViewMyJspaceHelper::getListPage($trie_tmp, $affmax, $emptymode, $search_restrict_acl, 1, $showmode1_query, $catid_list, $language);
	else
		$names = null;
	
	if ($showmode == 0 || $showmode == 2)
		$nbpages = modViewMyJspaceHelper::getNbPage($emptymode, $search_restrict_acl, 1, $catid_list, $language);
	else
		$nbpages = -1;
